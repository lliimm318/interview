...
이렇게는 불가하다. ( 하나의 파티션 당 하나의 컨슈머가 연결 되어야해서 )

- 메세지를 Batch 처리 가능
- 한 개의 컨슈머는 여러 개의 토픽을 처리 가능
- 메세지를 소비해도 메세지를 삭제하지 않는다. (Kafka delete policy에 의해 삭제)
- 카프카의 메시지 큐 시스템은 큐에서 메시지를 가져가도 사라지지 않기 때문에 여러 컨슈머 그룹이 동일 토픽에 붙을 수 있다
- 한 번 저장된 메세지는 여러 번 소비할 수 있다.
- 컨슈머는 컨슈머 그룹에 속하며, 파티션은 같은 컨슈머그룹의 여러 개의 컨슈머에 연결 불가
- 컨슈머 그룹은 컨슈머가 일정한 주기로 하는 하트비트(컨슈머가 poll()하거나 메시지의 오프셋을 커밋할때 보냄)를 통해 컨슈머가 메시지를 처리하고 있다는 것을 인지하며, 
  만약 오랫동안 하트비트가 없다면 해당 컨슈머의 세션이 타임아웃되고 리밸런스를 수행
...
모두 비동기 통신을 제공하며, Pub / Sub 모델 사용

### RabbitMQ
AMQP(Advanced Message Queuing Protocol, 클라이언트와 미들웨어간 메시지 교환 개방형 표준 프로토콜)을 위해 개발 되어 다른 AMQP 
프로토콜 기반 MQ(ex. RabbitMQ, ActiveMQ, ZeroMQ)등과 데이터 교환이 수월
...
- 분산형 아키텍처 (메시지 처리 순서 보장이 안됨)

#### 언제 사용할까?
- 메시지 처리 순서 보장되지 않아도 되는 경우 (분산형 아키텍처)
- 메시지 영속성을 보장하지 않아도 되는 경우. (저장기간 최대 7일)

### Kafka

#### 특징
- 고성능, 고가용성, 확장성
- 분산 처리 시스템으로서, 확장성(scalability) 및 고가용성(high available)이 높음 -> 노드 장애에 대한 대응성이 높음
- 배치 처리가 가능해 네트워크 I/O 오버헤드를 줄일 수 있음
...
#### Consumer 메시지 수신 처리 성능

![image](https://github.com/lliimm318/interview/assets/66578746/2364dd9a-a6ca-45d9-bc4c-709e8114cb05)
